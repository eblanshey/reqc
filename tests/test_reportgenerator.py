...
class TestReportGeneratorMissing:
    def test_missing_shown_with_prefix_and_source(self):
        """REQ: Output missing coverage prefixed with MISSING and include the source doc file path with line number"""
        gen = ReportGenerator()
        missing = [
            DocRequirement(text="The system shall authenticate users", source="docs/example.md:12"),
            DocRequirement(text="The system shall log errors", source="docs/example.md:15"),
        ]
        result = gen.generate(
            missing=missing,
            implemented=[],
        ...
        assert "[MISSING] docs/example.md:12: The system shall authenticate users" in result
        assert "[MISSING] docs/example.md:15: The system shall log errors" in result


class TestReportGeneratorStale:
    def test_stale_shown_with_prefix_and_source(self):
        """REQ: Output stale targets prefixed with STALE and include the source target file path with line number"""
        gen = ReportGenerator()
        TargetRequirement = namedtuple('TargetRequirement', ['text', 'source'])
        stale = [
            TargetRequirement(text="The system shall support LDAP", source="tests/test_auth.py:42"),
            TargetRequirement(text="The system shall support OAuth", source="tests/test_old.py:88"),
        ]
        result = gen.generate(
            missing=[],
            implemented=[],
        ...
        assert "[STALE] tests/test_auth.py:42: The system shall support LDAP" in result
        assert "[STALE] tests/test_old.py:88: The system shall support OAuth" in result
...
class TestReportGeneratorAllFlag:
    def test_all_flag_shows_implemented_section(self):
        ...
        implemented = [
            TargetRequirement(text="The system shall log errors", source="tests/test_auth.py:28"),
        ]
        stale = [
            TargetRequirement(text="The system shall support LDAP", source="tests/test_old.py:88"),
        ]
        result = gen.generate(
            missing=missing,
